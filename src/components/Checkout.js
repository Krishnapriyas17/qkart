  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange={(e)=>handleNewAddress((currNewAddress) => ({ ...currNewAddress, value: e.target.value, }))}
      />
      <Stack direction="row" my="1rem">
        ...
        <Button
          variant="text"
          onClick={()=>handleNewAddress((currNewAddress) => ({ ...currNewAddress, isAddingNewAddress: false, }))}
        >
          Cancel
        </Button>
      </Stack>
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      setNewAddress((prev)=>({...prev,isAddingNewAddress:false})) 
  ...
  const performCheckout = async (token, items, addresses) => {
    if(validateRequest(items,addresses)){ 
      ...
      if(response.data.success){ 
        ...
        let balance=localStorage.getItem("balance") 
        balance-=getTotalCartValue(items); 
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              <Box className={`address-item ${addresses.selected===item._id? "selected":"not-selected"}`} 
              onClick={()=>setAddresses({ ...addresses, selected: item._id })}> 
              <Typography> {item.address} </Typography> 
              <Button className="explore-button" variant="text" onClick={()=>deleteAddress(token,item._id)}> 
              <Delete/> DELETE </Button> 
              </Box> ) 
              : 
              <Typography my="1rem">No addresses found for this account. Please add one to proceed </Typography>} </Box>
